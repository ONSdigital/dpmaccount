...
make_codatamod_df.dpmaccount_datamod_norm <- function(mod, classif_vars) {
  ...
  codatamod_df$scale_sd <- scale_sd
  if (is_popn) {
    ...
    codatamod_df$scale_sd[is_accession] <- NA
  }
  ans <- nest_to_df(
    df = codatamod_df,
    nms_data = c(
...
make_codatamod_df.dpmaccount_datamod_t <- function(mod, classif_vars) {
  ...
  left_join <- function(x, y) merge(x, y, all.x = TRUE)
  codatamod_df <- left_join(classif_vars, data)
  codatamod_df$df <- df_param
  codatamod_df <- Reduce(left_join, list(codatamod_df, ratio, scale))
  codatamod_df$data <- codatamod_df$count
  codatamod_df$is_obs <- as.integer(!is.na(codatamod_df$data))
  ord <- with(codatamod_df, order(cohort, sex, time, age))
  codatamod_df <- codatamod_df[ord, ]
  ...
  if (is_popn) {
    is_accession <- with(codatamod_df, age == time - cohort - 1L)
    codatamod_df$ratio[is_accession] <- NA
    codatamod_df$scale[is_accession] <- NA
    codatamod_df$scale_ratio[is_accession] <- NA
  }
  ans <- nest_to_df(
    df = codatamod_df,
    nms_data = c("data", "is_obs", "df", "ratio", "scale", "scale_ratio", "time", "age"),
    nms_group = c("cohort", "sex"),
    nm_value = "args"
  )
  ans$nm_series <- nm_series
...
## HAS_TESTS
#' @export
make_codatamod_df.dpmaccount_datamod_nbinom <- function(mod, classif_vars) {
  data <- mod$data
  ratio <- mod$ratio
  disp <- mod$disp
  scale_ratio <- mod$scale_ratio
  ...
  left_join <- function(x, y) merge(x, y, all.x = TRUE)
  codatamod_df <- left_join(classif_vars, data)
  codatamod_df <- Reduce(left_join, list(codatamod_df, ratio, disp))
  codatamod_df$data <- codatamod_df$count
  codatamod_df$is_obs <- as.integer(!is.na(codatamod_df$data))
  ord <- with(codatamod_df, order(cohort, sex, time, age))
  codatamod_df <- codatamod_df[ord, ]
  ...
  ans <- nest_to_df(
    df = codatamod_df,
    nms_data = c("data", "is_obs", "ratio", "disp", "scale_ratio", "time", "age"),
    nms_group = c("cohort", "sex"),
    nm_value = "args"
  )
  ans$nm_series <- nm_series
  ans$nm_data <- nm_data
  ans$datamod <- lapply(ans$args, new_codatamod_nbinom)
  ans <- ans[-match("args", names(ans))]
  ans <- tibble::tibble(ans)
  ans
}
...
## HAS_TESTS
#' @export
make_codatamod_df.dpmaccount_datamod_poisson <- function(mod, classif_vars) {
  data <- mod$data
  ratio <- mod$ratio
  scale_ratio <- mod$scale_ratio
  nm_series <- mod$nm_series
  ...
  left_join <- function(x, y) merge(x, y, all.x = TRUE)
  codatamod_df <- left_join(classif_vars, data)
  codatamod_df <- Reduce(left_join, list(codatamod_df, ratio))
  codatamod_df$data <- codatamod_df$count
  codatamod_df$is_obs <- as.integer(!is.na(codatamod_df$data))
  ord <- with(codatamod_df, order(cohort, sex, time, age))
  codatamod_df <- codatamod_df[ord, ]
  ...
  ans <- nest_to_df(
    df = codatamod_df,
    nms_data = c("data", "is_obs", "ratio", "scale_ratio", "time", "age"),
    nms_group = c("cohort", "sex"),
    nm_value = "args"
  )
  ans$nm_series <- nm_series
  ans$nm_data <- nm_data
  ans$datamod <- lapply(ans$args, new_codatamod_poisson)
  ans <- ans[-match("args", names(ans))]
  ans <- tibble::tibble(ans)
  ans
}
...
#' @export
print.dpmaccount_datamod_nbinom <- function(x, ...) {
  nchar_offset <- 10
  cat("Negative-binomial data model : An object of class \"dpmaccount_datamod_nbinom\"\n\n")
  cat("dataset:", x$nm_data, "\n")
  cat("series:", x$nm_series, "\n")
  cat("data:\n")
  print(summary(x$data))
...
#' @export
print.dpmaccount_datamod_poisson <- function(x, ...) {
  nchar_offset <- 10
  cat("Poisson data model : An object of class \"dpmaccount_datamod_poisson\"\n\n")
  cat("dataset:", x$nm_data, "\n")
  cat("series:", x$nm_series, "\n")
  cat("data:\n")
  print(summary(x$data))
